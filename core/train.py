...
import torchvision
import random
from utils.eval_landmarks import *
...
def train(cfg, init_epoch, dataset_loader, train_transforms, val_transforms,
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        ...
        for seq_idx, (_, seq_blur, seq_clear, seq_landmark, seq_heatmap) in enumerate(train_data_loader):
            # Measure data time
            ...
            seq_heatmap = [utils.network_utils.var_or_cuda(heatmap).float() for heatmap in seq_heatmap]
            ...
            fusion_last_feature = None
            for batch_idx, [img_blur, img_clear, heatmap_clear] in enumerate(zip(seq_blur, seq_clear, seq_heatmap)):
                img_blur_hold = img_blur
                output_img, output_fea, heatmap, fusion_feature = deblurnet(img_blur, last_img_blur, output_last_img, output_last_fea, fusion_last_feature)

                # deblur loss
                deblur_mse_loss = 0.0
                deblur_mse_loss += mseLoss(output_img, img_clear)
                heatmap_weight = cfg.TRAIN.HEATMAP_WEIGHT
                deblur_mse_loss += heatmap_weight * mseLoss(heatmap, heatmap_clear)

                deblur_mse_losses.update(deblur_mse_loss.item(), cfg.CONST.TRAIN_BATCH_SIZE)
                if cfg.TRAIN.USE_PERCET_LOSS == True:
                    deblur_percept_loss = 0.0
                ...
                fusion_last_feature = fusion_feature.detach()
            # print per sequence
