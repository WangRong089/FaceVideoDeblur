...
class Compose(object):
    ...
    def __call__(self, seq_blur, seq_clear, seq_landmark):
        for t in self.transforms:
            seq_blur, seq_clear, seq_landmark = t(seq_blur, seq_clear, seq_landmark)
        return seq_blur, seq_clear, seq_landmark
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, seq_blur, seq_clear, seq_landmark):
        if random.random() < 0.5:
            ...
            size = seq_clear[0].shape[0]
            for landmark in seq_landmark:
                landmark[:, 0] = size - landmark[:, 0]
        return seq_blur, seq_clear, seq_landmark
...
class ToTensor(object):
    ...
    def __call__(self, seq_blur, seq_clear, seq_landmark):
        ...
        return seq_blur_tensor, seq_clear_tensor, seq_landmark
