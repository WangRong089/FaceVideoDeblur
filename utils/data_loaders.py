...
class VideoDeblurDataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, file_list_with_metadata, transforms = None):
        ...
        landmark_file = open(cfg.DIR.LANDMARK_INFO, 'rb')
        self.landmarks = pickle.load(landmark_file)
        landmark_file.close()
    ...
    def __getitem__(self, idx):
        if self.file_list[idx]['phase'] == 'train':
            name, seq_blur, seq_clear, seq_landmark = self.get_datum(idx)
            seq_blur, seq_clear, seq_landmark = self.transforms(seq_blur, seq_clear, seq_landmark)
            # heatmap is not needed in test
            seq_heatmap = [torch.from_numpy(np.ascontiguousarray(
                eval_landmarks.generate_gt(cfg.DATA.SIZE, landmark, cfg.DATA.SIGMA)))
                for landmark in seq_landmark]
            return name, seq_blur, seq_clear, seq_landmark, seq_heatmap
        else:
            name, seq_blur, seq_clear = self.get_datum(idx)
            # In test transform no flip is involved
            seq_blur, seq_clear, _ = self.transforms(seq_blur, seq_clear, None)
    ...
    def get_datum(self, idx):
        ...
        for i in range(length):
            ...
            frame = int(os.path.split(seq_blur_paths[i])[-1][:-4])
            if phase == 'train':
                seq_landmark.append(landmarks[frame])
        ...
        if phase == 'train' and random.random() < 0.5:
            ...
            seq_blur.reverse()
            seq_clear.reverse()
            seq_landmark.reverse()
        if phase == 'train':
            return name, seq_blur, seq_clear, seq_landmark
        else:
            return name, seq_blur, seq_clear
    ...
    def get_dataset(self, dataset_type, transforms=None):
        ...
        for file in self.files_list:
            if dataset_type == DatasetType.TRAIN and file['phase'] == 'train':
                ...
                # TODO: bug here, not loading the rest frames
