...
class FeatureHeatmapFusingBlock(nn.Module):
    ...
    def forward(self, feature, heatmap, debug=False):
        ...
        feature_o = self.conv_in(feature)
        feature_o = self.resnet(feature_o) # B * (num_heatmap*feat_channel_in) * h * w
        ...
        if debug:
            ...
            feature_o = feature_o.view(batch_size, self.num_heatmap, -1, w, h) * attention.unsqueeze(2)
            feature_o = feature_o.sum(1)
            return feature_o + feature
